...
function removeOldScores(){
    $cmd = "DELETE FROM scores ORDER BY score DESC LIMIT 20, 1000";
    begin();
    $res = mysql_query($cmd);
    if ($res){
        commit();
...
function addNewScore($name, $score){
    $ans = "ret=OK";
    $cmd = "INSERT INTO scores (name, score) VALUES ('$name', '$score')";
    begin();
    $res = mysql_query($cmd);
    if ($res){
        $ans .= "&name=$name&score=$score";
        commit();
    } else {
        rollback();
    }
...
function getTop10(){
    $ans = 'ret=OK';
    $cmd = "SELECT name, score FROM scores ORDER BY score DESC LIMIT 10";
    begin();
    $res= mysql_query($cmd);
    if ($res){
        commit();
